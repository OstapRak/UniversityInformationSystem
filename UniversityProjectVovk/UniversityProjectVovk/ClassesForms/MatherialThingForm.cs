...
namespace UniversityProjectVovk.ClassesForms
{
    public partial class MatherialThingForm : ThingForm
    {
        ...
        {
 	        base.open();
            string queryString = "SELECT Matherial FROM dbo.MatherialThing where Id = '" + (node.Tag as NodeData).Object.Id + "';";
        ...
        {
            base.delete();
            string queryString = "DELETE FROM dbo.MatherialThing where Id = '" + (node.Tag as NodeData).Object.Id + "';";
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                SqlCommand command = new SqlCommand(queryString, connection);
                try
