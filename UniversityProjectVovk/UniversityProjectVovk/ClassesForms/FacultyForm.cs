...
namespace UniversityProjectVovk.ClassesForms
{
    public partial class FacultyForm : GroupOfPeopleForm
    {
        ...
        {
 	         base.open();
             string queryString = "SELECT Dean FROM dbo.Faculty where Id = '" + (node.Tag as TObject).Id + "';";
        ...
        {
            base.delete();
            string queryString = "DELETE FROM dbo.Faculty where Id = '" + (node.Tag as TObject).Id + "';";
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                SqlCommand command = new SqlCommand(queryString, connection);
                try
