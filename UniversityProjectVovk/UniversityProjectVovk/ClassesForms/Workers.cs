...
namespace UniversityProjectVovk.ClassesForms
{
    public partial class Workers : PeopleForm
    {
        ...
        {
 	         base.open();
             string queryString = "SELECT Sallary, Position FROM dbo.Worker where Id = '" + (node.Tag as TObject).Id + "';";
        ...
        {
            base.delete();
            string queryString = "DELETE FROM dbo.Worker where Id = '" + (node.Tag as TObject).Id + "';";
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                SqlCommand command = new SqlCommand(queryString, connection);
                try
