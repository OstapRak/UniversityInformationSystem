...
namespace UniversityProjectVovk.ClassesForms
{
    public partial class CultureThingForm : ThingForm
    {
        ...
        {
 	         base.open();
             string queryString = "SELECT Description FROM dbo.CultureThing where Id = '" + (node.Tag as NodeData).Object.Id + "';";
        ...
        {
            base.delete();
            string queryString = "DELETE FROM dbo.CultureThing where Id = '" + (node.Tag as NodeData).Object.Id + "';";
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                SqlCommand command = new SqlCommand(queryString, connection);
                try
