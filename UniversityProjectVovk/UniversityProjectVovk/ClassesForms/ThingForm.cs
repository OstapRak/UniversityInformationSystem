...
namespace UniversityProjectVovk.ClassesForms
{
    public partial class ThingForm : ObjectForm
    {
        ...
        {
 	         base.open();
             string queryString = "SELECT Purpose FROM dbo.Thing where Id = '" + (node.Tag as TObject).Id + "';";
        ...
        {
            base.delete();
            string queryString = "DELETE FROM dbo.Thing where Id = '" + (node.Tag as TObject).Id + "';";
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                SqlCommand command = new SqlCommand(queryString, connection);
                try
