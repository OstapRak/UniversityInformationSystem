...
namespace UniversityProjectVovk.ClassesForms
{
    public partial class RersonalThingForm : MatherialThingForm
    {
        ...
        {
 	         base.open();
             string queryString = "SELECT Owner FROM dbo.PersonalThing where Id = '" + (node.Tag as NodeData).Object.Id + "';";
        ...
        {
            base.delete();
            string queryString = "DELETE FROM dbo.PersonalThing where Id = '" + (node.Tag as NodeData).Object.Id + "';";
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                SqlCommand command = new SqlCommand(queryString, connection);
                try
