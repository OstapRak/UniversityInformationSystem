...
namespace UniversityProjectVovk.ClassesForms
{
    public partial class AwardForm : ManageOfPeopleForm
    {
        ...
        {
            base.delete();
            string queryString = "DELETE FROM dbo.Award where Id = '" + (node.Tag as NodeData).Object.Id + "';";
        ...
        {
 	         base.open();
            string queryString = "SELECT Present FROM dbo.Award where Id = '" + (node.Tag as NodeData).Object.Id + "';";
            int newid = -1;
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                SqlCommand command = new SqlCommand(queryString, connection);
