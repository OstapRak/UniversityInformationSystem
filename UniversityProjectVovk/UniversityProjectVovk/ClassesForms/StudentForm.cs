...
namespace UniversityProjectVovk.ClassesForms
{
    public partial class StudentForm : PeopleForm
    {
        ...
        {
 	         base.open();
             string queryString = "SELECT IsPay, YearOfEntry FROM dbo.Student where Id = '" + (node.Tag as NodeData).Object.Id + "';";
        ...
        {
            base.delete();
            string queryString = "DELETE FROM dbo.Student where Id = '" + (node.Tag as NodeData).Object.Id + "';";
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                SqlCommand command = new SqlCommand(queryString, connection);
                try
