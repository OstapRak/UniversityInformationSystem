...
namespace UniversityProjectVovk.ClassesForms
{
    public partial class ManageOfPeopleForm : NotMatherialForm
    {
        ...
        {
 	         base.open();
             string queryString = "SELECT Person FROM dbo.ManageOfPeople where Id = '" + (node.Tag as NodeData).Object.Id + "';";
        ...
        {
            base.delete();
            string queryString = "DELETE FROM dbo.ManageOfPeople where Id = '" + (node.Tag as NodeData).Object.Id + "';";
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                SqlCommand command = new SqlCommand(queryString, connection);
                try
